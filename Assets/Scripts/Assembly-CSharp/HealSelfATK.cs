...
public class HealSelfATK : SpellScript
{
	...
	public new static int EvaluateLanePlacement(PlayerType player, Lane lane, CardItem item)
	{
		...
		CreatureScript creatureScript = CardScript.BestCandidateForHealing(candidates, -2, true);
		CardScript.ResetMods();
		CardScript.LaneMods[(int)player, creatureScript.CurrentLane.Index].DEF = Math.Min(creatureScript.Damage, creatureScript.ATK);
	...
	public override bool DoResult(CardScript target)
	{
		...
		creatureScript.Heal(creatureScript.ATK);
		return true;
	}
}
