...
public class ReduceDEF : SpellScript
{
	...
	public new static int EvaluateLanePlacement(PlayerType player, Lane lane, CardItem item)
	{
		...
		foreach (Lane item2 in lanes)
		{
			...
			if (item2.OpponentLane.HasCreature())
			{
				...
				CardScript.LaneMods[(int)(!player), creature2.CurrentLane.Index].DEF = creature2.DEF / 2;
			}
		}
	...
	{
		(target as CreatureScript).DEFFactor *= 0.5f;
		return true;
	}
}
