public class ATKBonusCards : BuildingScript
{
	...
	public new static int EvaluateLanePlacement(PlayerType player, Lane lane, CardItem item)
	{
		...
		if (lane.HasCreature())
		{
			...
			CardScript.LaneMods[(int)player, lane.Index].ATK = item.Val1 * cardsInHand;
	...
	public override void OnCardEnterPlay(CardScript script)
	{
		...
		if (script == this)
		{
			if (base.CurrentLane.HasCreature())
			{
				...
				ModifyATK(creature, base.Data.Val1 * cardsInHand);
				TriggerEffects();
			}
			Initialized = true;
		}
		else if (script.Data.Form.Type == CardType.Creature && script.CurrentLane == base.CurrentLane)
		{
			...
			ModifyATK(script2, base.Data.Val1 * cardsInHand);
			TriggerEffects();
		}
