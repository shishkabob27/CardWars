...
public class Polymorph : SpellScript
{
	...
	private static CreatureScript BestCandidateForSwap(CWList<Lane> Candidates, PlayerType player)
	{
		...
		foreach (CreatureScript item in list)
		{
			if (item.Enemy == null)
			{
				if (item.CanWin)
				{
					if (1 < GameState.Instance.GetHealth(player))
					{
						list2.Add(item);
					}
				}
		...
		foreach (CreatureScript item2 in list7)
		{
			...
			if (item2.ATK > creatureScript2.ATK)
			{
				creatureScript2 = item2;
			}
		}
	...
	public new static int EvaluateLanePlacement(PlayerType player, Lane lane, CardItem item)
	{
		...
		CardScript.LaneMods[(int)(!player), creatureScript.CurrentLane.Index].ATK = 1 - creatureScript.ATK;
		CardScript.LaneMods[(int)(!player), creatureScript.CurrentLane.Index].DEF = 1 - creatureScript.DEF;
	...
	public override bool DoResult(CardScript target)
	{
		...
		creatureScript.FloopDisabled = true;
		creatureScript.ATKMod = 1 - creatureScript.Data.ATK;
		creatureScript.DEFMod = 1 - creatureScript.Data.DEF;
		return true;
	}
}
